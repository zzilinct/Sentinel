...
const q = {
  history: db.prepare('SELECT kind, target, mode, scam, virus, malware, created_at FROM scan_history WHERE user_id = ? ORDER BY created_at DESC LIMIT ?'),
...
function register(router) {
  ...
  router.get('/api/v1/account/history', (req, res) => {
    ...
    sendJson(res, 200, {
      ...
      items: q.history.all(user.id, 50)
    });
  });
